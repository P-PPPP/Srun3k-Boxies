...
class MainWindow(QMainWindow):
    ...
    def __init__(self, config):
        ...
        self.init_account()
        if self.config['options']['autoMode']== True:
            self.login()
    ...
    def init_account(self):
        default = self.config['accounts']['account0']['username']
        password = self.config['accounts']['account0']['password']
    ...
    def save_config(self):
        ...
        if len(current_username) > 0:
            self.config['accounts']['account0']['username'] = current_username
            self.config['accounts']['account0']['password'] = current_password
    ...
    def autoRetry(self):
        ...
        if self.config['options']['autoMode'] == True:
            ...
            error = "error"
            ipIn = "ip_already"
            if error in result and ipIn in result == False:
                self.status.setText("retrying")
                if len(self.config['accounts']) > tried_times :
                    for n in range(len(self.config['accounts'])):
                        self.username_edit.setText(self.config['accounts']['account'+n]['username'])
                        self.password_edit.setText(self.config['accounts']['account'+n]['password'])
                        tried_times = n
                        self.login()
